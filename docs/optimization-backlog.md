...
| **A2** | **Tighter cell-grid cell size** (`cell ≈ cutoff`, not `2×cutoff`). Fewer candidate atoms visited per cell → directly cuts the dominant neighbor-build trip count. | Yes (same neighbor set) | Low | Watch the heterogeneous-radius stencil-coverage condition. Highest value/effort at tess 2. |
